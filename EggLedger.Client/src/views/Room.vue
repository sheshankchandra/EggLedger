  ...
  <div class="room-container">
    ...
    <div v-else class="room-content">
      ...
      <div class="room-header">
        <div class="room-info">
          <h1>{{ roomData.name }}</h1>
          <span class="room-code">Code: {{ roomData.code }}</span>
        </div>
...
const loadRoomData = async () => {
  try {
    ...
    const roomCode = sessionStorage.getItem('eggLedgerRoomCode')

    if (selectedRoom.id) {
      roomData.value = selectedRoom
    } else if (roomCode) {
      // Fetch room data by code if we don't have selectedRoom
...
const handleStock = async () => {
  try {
    ...
    await apiClient.post(`/egg-ledger-api/${roomData.value.code}/orders/stock`, payload)
    showNotification(`Successfully added ${stockForm.quantity} eggs!`)

    // Reset form
    Object.assign(stockForm, { name: '', quantity: 1, price: 0 })
