...
export const containerService = {
  ...
  createContainer(roomCode, containerData, signal) {
    return apiClient.post(`/egg-ledger-api/room/${roomCode}/container/create`, containerData, { signal });
  },

  // PUT /egg-ledger-api/room/{roomCode}/container/update/{id}
  updateContainer(roomCode, id, containerData, signal) {
    return apiClient.put(`/egg-ledger-api/room/${roomCode}/container/update/${id}`, containerData, { signal });
  },
  ...
  getPagedContainers(roomCode, page = 1, pageSize = 20, signal) {
    return apiClient.get(`/egg-ledger-api/room/${roomCode}/container/paged?page=${page}&pageSize=${pageSize}`, { signal });
  }
};
